...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "shell":
        ...
        return _run_nix_shell(installables, ["bash"], cwd=host_workdir, env=env)
...
def _copy_repo_snapshot(spec, workspace_root: Path) -> None:
    ...
    for relative_root in _snapshot_roots(spec):
        ...
        for root, dirs, files in os.walk(source_root, topdown=True):
            root_path = Path(root)
            dirs[:] = [name for name in dirs if not _is_excluded_path(root_path / name, excluded_roots)]
            destination_dir = workspace_root / root_path.relative_to(REPO_ROOT)
            destination_dir.mkdir(parents=True, exist_ok=True)
            for file_name in files:
                source_path = root_path / file_name
                if _is_excluded_path(source_path, excluded_roots):
                    continue
...
def _snapshot_roots(spec) -> list[Path]:
...
def _is_excluded_path(path: Path, excluded_roots: set[Path]) -> bool:
    if path == REPO_ROOT:
        return False
