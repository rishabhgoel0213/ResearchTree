...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Emit a generic objective JSON payload for Parameter Golf.")
    parser.add_argument(
        "repo_root",
        nargs="?",
        default=".",
    ...
    parser.add_argument(
        ...
        help="Path to the Parameter Golf score.py entrypoint.",
    )
...
def main() -> int:
    ...
    if args.data_path is not None:
        command.extend(["--data-path", str(args.data_path.resolve())])
    if args.tokenizer_path is not None:
        command.extend(["--tokenizer-path", str(args.tokenizer_path.resolve())])
    ...
    success = bool(payload.get("passes_submission_size_cap")) and bool(payload.get("passes_line_cap"))
    artifacts = {
        key: value
        for key, value in {
