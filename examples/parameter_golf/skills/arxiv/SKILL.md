...
Local execution in this Codex setup uses the bundled wrapper:
```bash
{baseDir}/scripts/run-arxiv.sh search "llm security" --max 5
```
The wrapper uses `uv` to provide the Python dependencies for `arxiv_tool.py` without requiring a global `pip` install.
